...
use crate::models::user::{ReqPageUserVo, User, DeleteUsers};
...
#[post("get_by_user_id")]
pub async fn get_by_user_id(arg:Json<User>,_request:HttpRequest,pool:Data<Pool<Sqlite>>) ->HttpResponse{
    if arg.user_id.is_none() {
       let err =    Api::<()>::error("userId 为空".to_string()).await.to_response_of_json().await;
       return err;
    };
...
#[post("/list_user")]
pub async fn list_user(user: Json<ReqPageUserVo>, _request: HttpRequest, pool: Data<Pool<Sqlite>>) -> HttpResponse {
...
#[post("delete_users")]
pub async fn delete_users(arg:Json<DeleteUsers>,_request:HttpRequest,pool:Data<Pool<Sqlite>>) -> HttpResponse{
