...
pub async fn login(request: HttpRequest) -> HttpResponse {
    ...
    Api::<String>::success().await.to_response_of_html(body).await
}

#[post("/login_submit")]
pub async fn login_submit(login_form: Json<LoginUserForm>, pool: Data<Pool<Sqlite>>, _request: HttpRequest) -> HttpResponse {
    let other_login = login_form.0;
    if other_login.user_name.is_none() || other_login.password.is_none() {
        return Api::<String>::error("未知的用户名和密码".to_string()).await.to_response_of_json().await;
    }

    //根据用户名查询用户数据
    let user_result = sqlx::query_as::<Sqlite, User>("select * from user where user_name = $1").bind(other_login.user_name.unwrap()).fetch_one(pool.as_ref()).await;

    if user_result.is_err() {
        return Api::<String>::error("数据服务错误".to_string()).await.to_response_of_json().await;
    }
    let user = user_result.unwrap();
    if &0 == &user.status.unwrap() {
        return Api::<String>::error("用户被禁用".to_string()).await.to_response_of_json().await;
    }
    if !user.clone().password.unwrap().eq(&other_login.password.unwrap()) {
        return Api::<String>::error("密码错误".to_string()).await.to_response_of_json().await;
    }
